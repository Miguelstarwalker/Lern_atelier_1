# Lern-Periode 1
Name: Ferreira Gomes Miguel

22.08.2025 bis 22.09.2024

## Grob-Planung
### Noten
Wo stehen Sie mit Ihren Noten? In welchen Modulen waren Sie besonders stark; in welchen sind die ungenügend? Welche davon sind besonders wichtig?
Noch keine Noten. Ich möchte mein Bestes geben.

### Veränderungen
Was möchten Sie generell im Vergleich zur letzten Lernperiode anpassen?
Erste Lern-Periode, ist nicht möglich zu verändern.

### Projekte / neue Technologien
Was für Projekte/neue Technologien möchten Sie gerne in dieser Lernperiode lernen?
C Sharp genauer ausführen und programmieren können, somit ich eventuell ohne hilfsmitteln (sei es Powerpoints oder anderes) programmieren könnte
und wissen wie man strukturiert plant (z.B. mit PAP).

### Generelle Ziele
Was haben Sie für klare und messbare Ziele in dieser Lernperiode?
Modul 319 Mission 10 vor dem Ferien schaffen somit ich nach den Ferien nur repetieren muss.
Modul 431 Handlungsziel 4 fertig machen vor den Ferien.
Reflexion 22.08.2025: Heute habe ich alle nötige Lernatelier aufgaben gemacht für heute, die Datein hatten keine Probleme, alles wurde erfolgreich beendet, Lesseverständnis verbessern, denn ich bei sehr viele Teilaufgaben es einfach nicht verstanden habe.

## Planung 12.09.2025
1. Als erstes möchte ich github verbessern mit Feedback von letzter Woche.
2. "Cheatsheet" für Modul 319 mit Saskia (evtl. auch Zehra) fertig machen (mindest. mehr Hinzufügen).
3. Modul 431 Aufgaben, welche ich am Dienstag gemacht habe, abegeben (wenn nicht Samstag letzter Tag ohne Ausnahmen).
## Reflexion 12.09.2025
-





alte Version der Planung als context.
# Lern_atelier_1
## 1. Tag
### Ziele
1. Als erstes möchte ich Modul 431 aufhollen(Mission 1, 2 und 3), denn ich am Dienstag Probleme mit einloggen des Moodles Account.
2. Nachdem mache ich weiter mit Modul 319 Mission 2, denn ich stecken geblieben bin bei Aufgabe 1.(Programmieren)
3. Als letztes möchte ich bei Modul 319 Mission 3 die Hälfte vervollständingen somit ich bei der nächste Mittwoch Iap Lektion der Plan besser folgen könnte.
# Lern_atelier_1
## 2. Tag
### Ziele
1. Ich möchte Abgabe Test-LB vervollständigen und abgeben.
2. Eine Lernperiode erstellen für diesen Quartal.
3. (Wenn genügend Zeit weiter machen mit Modul 319)
# Lern_atelier_1
## 3. Tag
### Ziele
1. Ich möchte alles von Lernatelier heute beenden, somit ich bei der "Selbst-Arbeit" starten kann.
2. Modul 431 aufgabe 11 vervollständigen, somit ich wieder im Tempo bin und nicht zurückfalle.
3. Modul 319 bis und mit Mission 6 somit ich mehr Zeit habe für Aufgabe 7 (laut einigen dauert es lange).
# Lern_atelier_1
## 4. Tag
### Ziele
1. Lerngespräch haben und durch dem reflektieren und Aarbeitsweise ändern wenn nötig.
2. Modul 319 Mission 7 (704) abgeben.
3. Modul 319 Mission 8 vervollständingen, somit ich mehr Zeit für 9 & 10 habe (laut einige ist es lange).
4. Modul 431 1712 erledigen, somit ich nicht zu langsam im Plan werde.


